/******************************************************************************
 * @file        utility.h
 * @brief       Declaration of the parameter class and related functions.
 *
 * @author      Yunxiang He
 * @date        2024-11-25
 * @version     2.0.0
 ...
 *
 * @details     This file contains the definition and assignment of the parameters of the slam module,
 *              and obtains the parameters read from the config file.
 *
 * @license     MIT License
 ******************************************************************************/
...
#include <arps/so3_math.h>
#include <arps/color.h>
#include <vector>
#include <deque>
#include <string>
#include <iostream>
        ...
        namespace arps_lidar_inertial_odometry {
            ...
            struct MeasureGroup // Lidar data and imu dates for the curent process
            {
                MeasureGroup() {
                    ...
                    //        this->surf_lidar.reset(new PointCloudXYZI());
                    imu.clear();
                };
                double lidar_beg_time;
                PointCloudXYZI::Ptr lidar;
                //    PointCloudXYZI::Ptr surf_lidar;
                std::deque<sensor_msgs::msg::Imu::ConstPtr> imu;
            };
            ...
            */
            ...
            template<typename T>
            void adjugateM3D(const Eigen::Matrix<T, 3, 3> &s, Eigen::Matrix<T, 3, 3> &t) {
                t << s(1, 1) * s(2, 2) - s(1, 2) * s(2, 1), s(1, 2) * s(2, 0) - s(1, 0) * s(2, 2), s(1, 0) * s(2, 1) -
                        s(1, 1) * s(2, 0),
                        s(0, 2) * s(2, 1) - s(0, 1) * s(2, 2), s(0, 0) * s(2, 2) - s(0, 2) * s(2, 0), s(0, 1) * s(2, 0)
                        - s(0, 0) * s(2, 1),
                        s(0, 1) * s(1, 2) - s(1, 1) * s(0, 2), s(0, 2) * s(1, 0) - s(0, 0) * s(1, 2), s(0, 0) * s(1, 1)
                        - s(0, 1) * s(1, 0);
            }

            inline double get_time_in_sec(const rclcpp::Time &time) {
                return time.seconds();
            }
