...
namespace arps {
    namespace slam {
        namespace arps_lidar_inertial_odometry {
            ...
            void Preprocess::process(const livox_ros_driver2::msg::CustomMsg::ConstPtr &msg,
            ...
            void Preprocess::process(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg,
                ...
                *pcl_out = pl_surf;
                // printf("Pt size = %lu, N_SCANS = %d\n", pcl_out->points.size(), N_SCANS);
                // static pcl::RadiusOutlierRemoval<PointType> outlierRemoval;
                // outlierRemoval.setInputCloud(pcl_out);
                // outlierRemoval.setRadiusSearch(0.5);
                // outlierRemoval.setMinNeighborsInRadius(3);
                // outlierRemoval.filter(*pcl_out);
            }
            ...
            void Preprocess::avia_handler(const livox_ros_driver2::msg::CustomMsg::ConstPtr &msg) {
                ...
                for (uint i = 0; i < plsize; i++) {
                    //            ROS_INFO_THROTTLE(1, "point tag is %d", msg->points[i].tag);
                    //            if ((msg->points[i].line < N_SCANS)  && (msg->points[i].tag == 0))
            ...
            void Preprocess::livox_handler(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg) {
                ...
                double base_time = get_time_in_sec(msg->header.stamp) * double(1e9);
                ...
                for (int i = 0; i < plsize; i++) {
                    if ((pl_orig.points[i].line < N_SCANS)) {
                        ...
                        point.curvature = (pl_orig.points[i].timestamp - base_time) / double(1e6);
                        // use curvature as time of each laser points
...
#define MAX_LINE_NUM 64
            ...
            void Preprocess::velodyne_handler(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg) {
                ...
                //    ROS_INFO_THROTTLE(1, "point size is %zu", pl_surf.points.size());
            }
            ...
            void Preprocess::xt32_handler(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg) {
                ...
                pcl::PointCloud<XT32Point> pl_orig;
                pcl::fromROSMsg(*msg, pl_orig);
                int plsize = pl_orig.points.size();
                pl_surf.reserve(plsize);
            ...
            void Preprocess::give_feature(pcl::PointCloud<PointType> &pl, std::vector<orgtype> &types) {
                ...
                for (uint i = head + 3; i < plsize2; i++) {
                    ...
                    } else if (types[i].edj[Prev] == Nr_zero && types[i].edj[Next] == Nr_nor && types[i - 1].dista >
                               0.0225 &&
                ...
                for (uint i = head + 1; i < plsize2; i++) {
                    if (types[i].range < blind_sqr || types[i - 1].range < blind_sqr || types[i + 1].range <
                        blind_sqr) {
                        continue;
                    }
            ...
            int Preprocess::plane_judge(const PointCloudXYZI &pl, std::vector<orgtype> &types, uint i_cur,
                                        uint &i_nex,
            ...
            bool Preprocess::edge_jump_judge(const PointCloudXYZI &pl, std::vector<orgtype> &types, uint i,
                                             Surround nor_dir) {
                if (nor_dir == 0) {
