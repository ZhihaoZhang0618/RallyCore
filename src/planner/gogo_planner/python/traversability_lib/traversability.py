...
class TraversabilityAnalyzerWithBGK_GPU:
    def __init__(self, resolution=0.2, x_length=5, y_length=5, time_window=600.0, max_history_frames=3):
        """
        初始化类，设定地图的基本参数。
    ...
    def spatial_temporal_bgk_inference(self, grid_cells, kernel_function, variance_weight, radius=2.0):
        ...
        for cell, history in self.historical_observations.items():
            for value, variance, timestamp in history:
                time_weight = np.exp(-(current_time - timestamp) / self.time_window)  # 时间衰减权重
        ...
        weights *= observed_weights  # 乘上时间权重
    ...
    def calculate_traversability(self, slope, flatness, step_height, uncertainties, position, w_slope, w_flatness, w_step_height):
        ...
        min_num, max_num = np.min(traversability), np.max(traversability)
